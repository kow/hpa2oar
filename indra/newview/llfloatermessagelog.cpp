...
LLToggleableMenu* LLFloaterMessageLog::mFilterMenu = NULL;
...
{
	sInstance = static_cast<LLFloaterMessageLog*>(LLFloaterReg::showInstance("message_log"));
}
void LLFloaterMessageLog::onClickFilterMenu(const LLSD& user_data)
{
	//llinfos << user_data.asString() << llendl;
	childSetText("filter_edit", user_data.asString());
...
BOOL LLFloaterMessageLog::postBuild()
{
	...
	registrar.add("MessageLog.Filter.Action",boost::bind(&LLFloaterMessageLog::onClickFilterMenu,this,  _2));
	mFilterMenu = LLUICtrlFactory::getInstance()->createFromFile<
			LLToggleableMenu> ("menu_filter_choice.xml", gMenuHolder,
...
void LLFloaterMessageLog::refreshNetInfo(BOOL force)
{
	...
	if(selected_itemp)
	{
		if(!force && getChild<LLTextEditor>("net_info")->hasSelection()) return;
...
{
	mFilterMenu->updateParent(LLMenuGL::sMenuContainer);
	mFilterMenu->setCanTearOff(FALSE);
	LLView* buttonp = sInstance->getChild<LLView>("filter_choice_btn");
	S32 x = buttonp->getRect().mLeft;
	S32 y = buttonp->getRect().mBottom;
	LLMenuGL::showPopup(sInstance, mFilterMenu, x, y);
}
// static
