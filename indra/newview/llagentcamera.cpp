...
void LLAgentCamera::cameraOrbitIn(const F32 meters)
{
	...
	{
		static LLCachedControl<F32> CameraOffsetScale(gSavedSettings,"CameraOffsetScale");
		F32 camera_offset_dist = llmax(0.001f, getCameraOffsetInitial().magVec() * CameraOffsetScale);
		
		mCameraZoomFraction = (mTargetCameraDistance - meters) / camera_offset_dist;
		static LLCachedControl<bool> FreezeTime(gSavedSettings,"FreezeTime");
		if (!FreezeTime && mCameraZoomFraction < MIN_ZOOM_FRACTION && meters > 0.f)
		{
...
void LLAgentCamera::updateLookAt(const S32 mouse_x, const S32 mouse_y)
{
	...
	else
	{
		...
		else if (cameraThirdPerson())
		{
			...
			static LLCachedControl<F32> YawFromMousePosition(gSavedSettings,"YawFromMousePosition");
			frameCamera.yaw( - x_from_center * YawFromMousePosition * DEG_TO_RAD);
			static LLCachedControl<F32> PitchFromMousePosition(gSavedSettings,"PitchFromMousePosition");
			frameCamera.pitch( - y_from_center * PitchFromMousePosition * DEG_TO_RAD);
...
void LLAgentCamera::updateCamera()
{
	...
	if (TRUE)
	{
		...
		if (cameraThirdPerson() && !mCameraSmoothingStop)
		{
			...
			static LLCachedControl<F32> CameraPositionSmoothing(gSavedSettings,"CameraPositionSmoothing");
			F32 smoothing = LLCriticalDamp::getInterpolant((F32)CameraPositionSmoothing * SMOOTHING_HALF_LIFE, FALSE);
...
LLVector3d LLAgentCamera::calcCameraPositionTargetGlobal(BOOL *hit_limit)
{
	...
	else if (mCameraMode == CAMERA_MODE_THIRD_PERSON && mFocusOnAvatar)
	{
		...
		{
			static LLCachedControl<F32> CameraOffsetScale(gSavedSettings,"CameraOffsetScale");
			local_camera_offset = mCameraZoomFraction * getCameraOffsetInitial() * CameraOffsetScale;
			...
			if (isAgentAvatarValid())
			{
				...
				if(!mCameraAnimating && gAgentAvatarp->mInAir && time_in_air > GROUND_TO_AIR_CAMERA_TRANSITION_START_TIME)
				{
					...
					{
						static LLCachedControl<F32> DynamicCameraStrength(gSavedSettings,"DynamicCameraStrength");
						target_lag = vel * DynamicCameraStrength / 30.f;
					}
	...
	static LLCachedControl<bool> DisableCameraConstraints(gSavedSettings,"DisableCameraConstraints");
	if (!DisableCameraConstraints && !gAgent.isGodlike())
	{
...
void LLAgentCamera::handleScrollWheel(S32 clicks)
{
	...
	else
	{
		...
		else if (mFocusOnAvatar && (mCameraMode == CAMERA_MODE_THIRD_PERSON))
		{
			...
			static LLCachedControl<F32> CameraOffsetScale(gSavedSettings,"CameraOffsetScale");
			F32 current_zoom_fraction = mTargetCameraDistance / (camera_offset_initial_mag * CameraOffsetScale);
			current_zoom_fraction *= 1.f - pow(ROOT_ROOT_TWO, clicks);
			cameraOrbitIn(current_zoom_fraction * camera_offset_initial_mag * CameraOffsetScale);
		}
		else
		{
