										   ...
										   const std::string& pname)
{
	...
	// <edit> This is probably OK because our Inv root should be created for us already, we're not creating it
	if(parent_id != LLUUID::null && parent_id != gLocalInventoryRoot && !gInventory.isObjectDescendentOf(parent_id, gLocalInventoryRoot))
	{
...
//</edit>
{
	...
	while(!feof(file) && fgets(buffer, MAX_STRING, file)) 
	{
		...
		if(0 == strcmp("inv_cache_version", keyword))
		{
			...
			if ((1 == succ) && (version == sCurrentInvCacheVersion) || ignore_version) //<edit />
			{
				// Cache is up to date
		...
		{
			if (is_cache_obsolete && !ignore_version) //<edit />
				break;
...
//<edit> wrapper around the new version for old code and cool people
//also ignores inventory cache version
bool LLInventoryModel::loadFromFile(const std::string& filename,
									LLInventoryModel::cat_array_t& categories,
									LLInventoryModel::item_array_t& items)
{
	bool ll_sucks_gay_dicks = true;
	loadFromFile(filename, categories, items, ll_sucks_gay_dicks, true);
	return true; //we don't really care whether the cache is obsolete in this case
}
//</edit>
