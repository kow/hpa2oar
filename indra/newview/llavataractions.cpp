...
#include "llworld.h"			// for regions we are connected to
...
void LLAvatarActions::teleportTo(const LLUUID& id)
{
			...
			iter != LLWorld::getInstance()->getRegionList().end(); ++iter)
	{
		...
		{
			S32 index = regionp->mMapAvatarIDs.find(id);
			if(index != -1)
			{
				found = true;
				pos = LLWorld::unpackLocalToGlobalPosition(regionp->mMapAvatars.get(index), origin_global);
				break;
			}
		}
	}
...
// static
bool LLAvatarActions::isNearby(const LLUUID& id)
{
	for (LLWorld::region_list_t::const_iterator iter = LLWorld::getInstance()->getRegionList().begin();
			iter != LLWorld::getInstance()->getRegionList().end(); ++iter)
	{
