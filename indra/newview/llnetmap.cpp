...
#include "llmutelist.h" // this is for showing lindens and muted avatars on the mini-map
#include "lltracker.h"
#include "llsurface.h"
...
void LLNetMap::draw()
{
	...
	static LLUIColor map_avatar_linden_color = LLUIColorTable::instance().getColor("MapAvatarLindenColor", LLColor4::white);
	static LLUIColor map_avatar_estate_owner_color = LLUIColorTable::instance().getColor("MapAvatarEstateOwnerColor", LLColor4::white);
	static LLUIColor map_avatar_muted_color = LLUIColorTable::instance().getColor("MapAvatarMutedColor", LLColor4::white);
	...
	
	{
			 ...
			 iter != LLWorld::getInstance()->getRegionList().end(); ++iter)
		{
			...
			for (i = 0; i < count; i++)
			{
				...
				LLColor4 avColor = map_avatar_color;

				if( i < regionp->mMapAvatarIDs.count())
				{
					std::string avName;

					gCacheName->getFullName(regionp->mMapAvatarIDs.get(i), avName);

					LLUUID estate_owner = LLWorld::getInstance()->getRegionFromPosGlobal(pos_global)->getOwner();
	
					//Lindens are always more Linden than your friend, make that take precedence
					if(LLMuteList::getInstance()->isLinden(avName))
					{
						avColor = map_avatar_linden_color;
					}
					//check if they are an estate owner at their current position
					else if(estate_owner.notNull() && regionp->mMapAvatarIDs.get(i) == estate_owner)
					{
						avColor = map_avatar_estate_owner_color;
					}
					//without these dots, SL would suck.
					else if(LLAvatarTracker::instance().getBuddyInfo(regionp->mMapAvatarIDs.get(i)) != NULL)
					{
						avColor = map_avatar_friend_color;
					}
					//big fat jerkface who is probably a jerk, display them as such.
					else if(LLMuteList::getInstance()->isMuted(regionp->mMapAvatarIDs.get(i)))
					{
						avColor = map_avatar_muted_color;
					}
				}
