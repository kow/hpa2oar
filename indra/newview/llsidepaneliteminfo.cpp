...
void LLSidepanelItemInfo::refreshFromItem(LLViewerInventoryItem* item)
{
	...
	childSetText("EditItemFolder", item->getParentUUID().asString());
	childSetText("EditItemGroup", item->getPermissions().getGroup().asString());
	childSetText("EditItemAssetID", item->getAssetUUID().asString());

	std::string type_str = LLAssetType::lookup(item->getType());
	if(type_str.c_str() == NULL) type_str = llformat("%d", item->getType());
	childSetText("EditItemType", type_str);
	...
	childSetText("EditItemInvType", invtype_str);
	childSetText("EditItemFlags", llformat("%d", item->getFlags()));
