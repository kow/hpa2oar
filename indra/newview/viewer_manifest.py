...
class WindowsManifest(ViewerManifest):
    ...
    def test_for_no_msvcrt_manifest_and_copy_action(self, src, dst):
        ...
        if src and (os.path.exists(src) or os.path.islink(src)):
            ...
            if not os.path.isdir(src):
                try:
                    crtversion=self.args['vcversion'].replace('.','') # change 8.0 into 80 etc.
                    if(self.args['configuration'].lower() == 'debug'):
                        test_assembly_binding(src, "Microsoft.VC%s.DebugCRT" % crtversion, "",self.args['vcversion'])
                    else:
                        test_assembly_binding(src, "Microsoft.VC%s.CRT" % crtversion, "",self.args['vcversion'])
    ...
    def construct(self):
        ...
        if self.prefix(src=os.path.join(os.pardir, 'sharedlibs', self.args['configuration']),
            ...
            if self.args['configuration'].lower() == 'debug':
                self.path("msvcr%sd.dll" % crtversion)
                self.path("msvcp%sd.dll" % crtversion)
                self.path("Microsoft.VC%s.DebugCRT.manifest" % crtversion)
            else:
                self.path("msvcr%s.dll" % crtversion)
                self.path("msvcp%s.dll" % crtversion)
                self.path("Microsoft.VC%s.CRT.manifest" % crtversion)
