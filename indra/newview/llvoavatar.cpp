...
void LLVOAvatar::idleUpdateNameTag(const LLVector3& root_pos_last)
{
	...
	const F32 NAME_SHOW_TIME = RenderNameShowTime;	// seconds
	static LLCachedControl<F32> RenderNameFadeDuration(gSavedSettings,"RenderNameFadeDuration");	
	const F32 FADE_DURATION = RenderNameFadeDuration; // seconds
	...
	if ( render_name )
	{
		...
		// If alpha > 0, create mNameText if necessary, otherwise delete it
		{
			...
			if (alpha > 0.f)
			{
				...
				static LLCachedControl<bool> use_tag_color(gSavedSettings, "_nacl_ShowTagColors");
				
				LLTextureEntry* texentry = getTE(0);
				if(use_tag_color && texentry->getGlow() > 0.0f)
				{
		...
		if (mNameText.notNull() && firstname && lastname)
		{
				...
				)
			{
				...
				static LLCachedControl<bool> use_tag_client(gSavedSettings, "_nacl_ShowTagClients");
				if(use_tag_client && !mClientTag.empty())
				{
					line += " (" + mClientTag + ")";
				}
				mNameAway = is_away;
...
LLUUID LLVOAvatar::remapMotionID(const LLUUID& id)
{
	...
	static LLCachedControl<bool> UseNewWalkRun(gSavedSettings,"UseNewWalkRun");
	BOOL use_new_walk_run = UseNewWalkRun;
	LLUUID result = id;
...
{
  	static LLCachedControl<bool> RenderUnloadedAvatar(gSavedSettings,"RenderUnloadedAvatar");
	if (RenderUnloadedAvatar)
		return TRUE;
	else
		return mFullyLoaded;
}
