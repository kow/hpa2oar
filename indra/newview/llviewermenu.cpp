...
class LLWorldForceSitOrStand : public view_listener_t
{
	bool handleEvent(const LLSD& userdata)
	{
		if(gAgentAvatarp)
...
class LLViewCheckShowHoverTips : public view_listener_t
{
	bool handleEvent(const LLSD& userdata)
	{
		static LLCachedControl<bool> ShowHoverTips(gSavedSettings,"ShowHoverTips");
		bool new_value = ShowHoverTips;
		return new_value;
	}
};
...
void initialize_menus()
{
	...
	view_listener_t::addMenu(new LLWorldForceSitOrStand(), "World.ForceSitOrStand");
