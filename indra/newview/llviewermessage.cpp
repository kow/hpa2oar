	...
	if (is_audible)
	{
		...
		if (chatter && chatter->isAvatar())
		{
			...
			if (!is_muted && !is_busy)
			{
				...
				visible_in_chat_bubble = UseChatBubbles;
...
{
	static LLCachedControl<bool> FallThroughTeleport(gSavedSettings, "FallThroughTeleport");
	if (gAgent.getTeleportState() != LLAgent::TELEPORT_NONE && !FallThroughTeleport)
	{
	...
	// <edit>
	static LLCachedControl<bool> Nimble(gSavedSettings, "Nimble");
	if(Nimble)
	{
...
{
	static LLCachedControl<bool> spoof_prot_alert(gSavedSettings, "SpoofProtectionAlerts");
	if(spoof_prot_alert)
	{
		LLSD args;
