...
#include "llviewernetwork.h" // for spoof protection
...
{
	static LLCachedControl<U32> PreferredMaturity(gSavedSettings,"PreferredMaturity");
	U8 sim_access = PreferredMaturity;
...
// <edit> Yeti Prefs needs special attention for IP Spoof protection
static LLRegisterPanelClassWrapper<LLPanelPreferenceYeti> t_pref_yeti("panel_preference_yeti");

BOOL LLPanelPreferenceYeti::postBuild()
{
	mSpoofProtectionAtOpen = gSavedSettings.getU32("SpoofProtectionLevel");
	//llinfos << "Spoof Protection At Open" << mSpoofProtectionAtOpen << llendl;
	return LLPanelPreference::postBuild();
}
void LLPanelPreferenceYeti::draw()
{
	LLPanelPreference::draw();
}
void LLPanelPreferenceYeti::apply()
{
	//llinfos << "Yeti apply" << llendl;
	U32 new_spoof_protection = gSavedSettings.getU32("SpoofProtectionLevel");
	//llinfos << "Spoof Protection At Apply" << new_spoof_protection << llendl;
...
void LLPanelPreferenceYeti::cancel()
{
	resetDirtyChilds();
	LLPanelPreference::cancel();
}
void LLPanelPreferenceYeti::saveSettings()
{
	//llinfos << "Yeti saveSettings" << llendl;
	resetDirtyChilds();
	LLPanelPreference::saveSettings();
}
bool LLPanelPreferenceYeti::hasDirtyChilds()
{
	std::list<LLView*> view_stack;
	view_stack.push_back(this);
	while(!view_stack.empty())
...
void LLPanelPreferenceYeti::resetDirtyChilds()
{
	std::list<LLView*> view_stack;
	view_stack.push_back(this);
	while(!view_stack.empty())
