...
// <edit>
LLUUID gLocalInventoryRoot = LLUUID::null;
// </edit>
...
const char* const VIEWER_WINDOW_CLASSNAME = "Yeti";
...
void idle_afk_check()
{
	...
	if (AFKTimeout && (gAwayTriggerTimer.getElapsedTimeF32() > AFKTimeout))
	{
		gAgent.setAFK();
	}
}
		...
		try
		{
			...
			// Sleep and run background threads
			{
						...
						|| !gFocusMgr.getAppHasFocus())
				{
					...
					static LLCachedControl<S32> BackgroundYieldTime(gSavedSettings,"BackgroundYieldTime");
					S32 milliseconds_to_sleep = llclamp((S32)BackgroundYieldTime, 0, 1000);
...
bool LLAppViewer::onQuitViewer(const LLSD& notification, const LLSD& response)
{
	S32 option = LLNotificationsUtil::getSelectedOption(notification, response);
	if(option == 0)
	{
...
bool onConfirmQuitBadNetworkHandler(const LLSD& notification, const LLSD& response)
{
	return LLAppViewer::instance()->onQuitViewer(notification,response);
}
void LLAppViewer::badNetworkHandler()
{
	LLSD args;
	args["MESSAGE"] = "Do you want to force disconnect because of a bad network handler?";
	LLNotificationsUtil::add("GenericAlert", args,LLSD(), onConfirmQuitBadNetworkHandler);
}
