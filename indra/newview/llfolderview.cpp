...
void LLFolderView::filter( LLInventoryFilter& filter )
{
	...
	static LLCachedControl<S32> FilterItemsPerFrame(gSavedSettings,"FilterItemsPerFrame");	
	filter.setFilterCount(llclamp((S32)FilterItemsPerFrame, 1, 5000));
...
void LLFolderView::draw()
{
	...
	static LLCachedControl<F32> TypeAheadTimeout(gSavedSettings,"TypeAheadTimeout");
...
BOOL LLFolderView::handleUnicodeCharHere(llwchar uni_char)
{
	...
	if (gFocusMgr.childHasKeyboardFocus(getRoot()))
	{
		...
		//do text search
		static LLCachedControl<F32> TypeAheadTimeout(gSavedSettings,"TypeAheadTimeout");
		if (mSearchTimer.getElapsedTimeF32() > TypeAheadTimeout)
		{
			mSearchString.clear();
		}
		mSearchTimer.reset();
...
void LLFolderView::doIdle()
{
	...
	static LLCachedControl<bool> DebugInventoryFilters(gSavedSettings,"DebugInventoryFilters");
	BOOL debug_filters = DebugInventoryFilters;
