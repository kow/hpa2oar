...
{
	//<edit> make sure not to make it so we can't remove local items
...
{
	LLViewerInventoryItem* item = static_cast<LLViewerInventoryItem*>(getItem());
	if(item && !item->isFinished() && !(gInventory.isObjectDescendentOf(mUUID, gLocalInventoryRoot))) //<edit /> local inventory fixes
	{
