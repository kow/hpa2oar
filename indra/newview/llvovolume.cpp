...
BOOL LLVOVolume::updateGeometry(LLDrawable *drawable)
{
	...
	if (mVolumeImpl != NULL)
	{
		...
		mVolumeSurfaceArea = getVolume()->sculptGetSurfaceArea(); // Poorly named. Works for everything.
		updateFaceFlags();
		return res;
	}
...
void LLVolumeGeometryManager::rebuildGeom(LLSpatialGroup* group)
{
	...
	U32 max_vertices = (RenderMaxVBOSize*1024)/LLVertexBuffer::calcStride(group->mSpatialPartition->mVertexDataMask);
	static LLCachedControl<S32> RenderMaxNodeSize(gSavedSettings,"RenderMaxNodeSize");
	U32 max_total = (RenderMaxNodeSize*1024)/LLVertexBuffer::calcStride(group->mSpatialPartition->mVertexDataMask);
