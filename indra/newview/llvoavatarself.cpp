...
{
  	static LLCachedControl<U32> TextureDiscardLevel(gSavedSettings,"TextureDiscardLevel");
	const U32 desired_tex_discard_level = TextureDiscardLevel; 
...
const std::string LLVOAvatarSelf::debugDumpAllLocalTextureDataInfo() const
{
	std::string text;
	static LLCachedControl<U32> TextureDiscardLevel(gSavedSettings,"TextureDiscardLevel");
	const U32 override_tex_discard_level = TextureDiscardLevel;
...
bool LLVOAvatarSelf::sendAppearanceMessage(LLMessageSystem *mesgsys) const
{
	...
	static LLCachedControl<bool> set_client_tag(gSavedSettings, "_nacl_SendTag");
	if(set_client_tag)
	{
		LLTextureEntry* entry = getTE(0);
		//You edit this to change the tag in your client. Yes.
		const char* tag_client = "Yeti"; //actually, does this really need the brackets? waste of two chars!
		LLUUID client_name;
		strncpy((char*)&client_name.mData[0], tag_client, UUID_BYTES);
		static LLCachedControl<LLColor4> tag_color(gSavedSettings, "_nacl_TagColor", LLColor4::magenta1);
