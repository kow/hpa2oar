...
void LLFloaterMessageBuilder::show(std::string initial_text)
{
	...
	LLFloaterReg::showInstance("message_builder",initial_text.length() > 0 ? LLSD().with("initial_text", initial_text) : LLSD());
}
...
void alert(const std::string& lol)
{
	LLSD args;
	args["MESSAGE"] = lol;
...
void LLFloaterMessageBuilder::onClickSend(void* user_data)
{
	...
	if(!lines.size())
	{
		alert("Not enough information :O");
		return;
	}
	...
	{
		alert("Expected direction 'in' or 'out'");
		return;
	}
	// Message
	...
	if(tokens.size() > 1)
	{
		if(tokens.size() > 2)
		{
			alert("Unexpected extra stuff at the top");
			return;
		}
		message = tokens[1];
	...
	if(lines.size() > 1)
	{
		...
		for( ; line_iter != line_end; ++line_iter)
		{
			...
			else
			{
				...
				{
					alert("Unexpected field when no block yet");
					return;
				}
				int eqpos = line.find("=");
				if(eqpos == line.npos)
				{
					alert("Missing an equal sign");
					return;
				}
	...
	if(template_iter == gMessageSystem->mMessageTemplates.end())
	{
		alert(llformat("Don't know how to build a '%s' message", message.c_str()));
		return;
	}
	LLMessageTemplate* temp = (*template_iter).second;
		 ...
		 blocks_iter != blocks_end; )
	{
		...
		if(parts_iter == parts_end)
		{
			if(block->mType != MBT_VARIABLE)
				alert(llformat("Expected '%s' block", block_name));
			else
			{
				++blocks_iter;
				continue;
			 ...
			 var_iter != var_end; ++var_iter)
		{
			...
			{
				alert(llformat("Expected '%s' field under '%s' block", var_name, block_name));
				return;
			}
		...
		if(part_var_iter != part_var_end)
		{
			alert(llformat("Unexpected field(s) at end of '%s' block", block_name));
			return;
		}
		++parts_iter;
		// test
	...
	if(parts_iter != parts_end)
	{
		alert("Unexpected block(s) at end");
		return;
	}
	// Build and send
	...
	for(parts_iter = parts.begin(); parts_iter != parts_end; ++parts_iter)
	{
			...
			part_var_iter != part_var_end; ++part_var_iter)
		{
			...
			{
				alert(llformat("Error adding the provided data for %s '%s' to '%s' block", mvtstr(pv.var_type).c_str(), pv.name.c_str(), block_name));
	...
	if(selected_itemp)
	{
		...
		{
			alert("No sending messages through dead circuits!");
			return;
		}
		if(outgoing)
		{
